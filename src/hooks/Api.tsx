...
export const useWeather = (): WeatherData | null => {
  ...
  // ---------------------------------------------------------
  const askForCity = async (): Promise<{
    lat: number;
    lon: number;
    city: string;
  }> => {
    const city =
      prompt("Unable to get location.\nEnter your city:") || "New York";
    ...
    try {
      ...
      if (data.length === 0) {
        return { lat: 40.7128, lon: -74.006, city: "New York, NY" };
      }

      const properCity = data[0].display_name.split(",")[0] || city;
    ...
    const run = async () => {
      ...
      try {
        ...
        setWeather({
          ...
          timestamp: `As of ${formattedTime}, ${formattedDate}`,
          city,
        });
      } catch (err) {
        console.error("Weather fetch failed, using placeholder:", err);
